function [ hrirs ] = lowp_kaiser(hrirs, fs,band_edge, band_stop)

% A = [0 1 0];                % band type: 0='stop', 1='pass'
% dev = [0.0001 10^(0.1/20)-1 0.0001]; % ripple/attenuation spec
% [M,Wn,beta,typ] = kaiserord(F,A,dev,44100);  % window parameters
% b = fir1(M,Wn,typ,kaiser(M+1,beta),'noscale'); % filter design

c = kaiserord( [band_edge band_stop], [1 0], [0.01 0.1], fs, 'cell' );
b = fir1(c{:});

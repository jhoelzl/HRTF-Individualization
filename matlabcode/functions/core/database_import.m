function database = database_import(db,importmode,use_drirs)
...
% use_drirs     optional: 0= hrirs or 1=drirs when it is available in the database

% OUTPUT
% database.
% data          HRIR Data of whole db
% subjects      vector of subject_ids
% angles        Matrix with Angle Entries
% data_dtf      only in UNIVERSAL db: directional transfer functions
% id         database id(string)
% name       database name
% department database department
% anthro     1 when db contains anthropometric dimension, otherwise 0
% fs         database hrir sampling rate
...
% Save
database.hrirs = DATA;
database.drirs = DATA_DTF;
database.fs = db_fs;
database.subjects = subjects;
database.anthro = db_anthro;
database.data_anthro = anthropometric;
database.angles = DATA_ANGLES;
database.id = db_id;
database.name = lower(db);
database.department = db_department;
end
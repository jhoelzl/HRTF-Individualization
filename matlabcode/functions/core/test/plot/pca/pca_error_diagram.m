...
for db=1:length(dbs)
    ...
    % Load Error File
    error_data = sprintf('../matlabdata/test_pca/variance_error_pca_%s.mat',dbs{db});
...
%                                 grid on
                                ...
                                saveas(9,sprintf('../thesis/images/test_pca/hist_pcs/pc%i/%s_%s_ipm%i_ips%i_em%i_ear%s_sm%i_pd',conf.pc_numbers(pc),dbs{db},conf.error_type{error_files(e_name)},conf.input_modes(im),conf.input_structures(is),conf.ear_modes(em),name_ear,conf.smoothing(sm)),'epsc');
                           ...
                           % saveas(11,sprintf('../thesis/images/test_pca/boxplot/%s_%s_ipm%i_ips%i_em%i_ear%s_sm%i',dbs{db},conf.error_type{error_files(e_name)},conf.input_modes(im),conf.input_structures(is),conf.ear_modes(em),name_ear,conf.smoothing(sm)),'epsc');

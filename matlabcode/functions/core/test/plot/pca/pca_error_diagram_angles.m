function pca_error_diagram_angles
    ...
    for e_name = 1:length(error_files)
        ...
        for im = 1:length(conf.input_modes)
            for is = 1:length(conf.input_structures) 
                for sm = 2;%1:length(conf.smoothing)  
                    for em = 1:length(conf.ear_modes)   
                        for ear = 1:length(conf.ears)
                            ...
                            saveas(11,sprintf('../thesis/images/test_pca/angle/%s_%s_ipm%i_ips%i_em%i_ear%s_sm%i_pc%i_b2',dbs{db},conf.error_type{error_files(e_name)},conf.input_modes(im),conf.input_structures(is),conf.ear_modes(em),name_ear,conf.smoothing(sm),conf.pc_numbers(pc)),'epsc');

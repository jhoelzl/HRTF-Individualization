function [ hrir_reconstr_left,hrir_reconstr_right ] = reconstr_pca3( hObject,eventdata,handles,weights_reconstruct )
...
if (pca_mode <= 2) % PCA DTF MODE
    ...
    % DTF    
    reconstruct = (pcs(:,1:select_pcs) * weights_reconstruct(:,1:select_pcs)')' + mn;

    half_points = size(reconstruct,2)/2;

    dtf_reconstruct_left = reconstruct(1:half_points);
    dtf_reconstruct_right = reconstruct(half_points+1:end);
    
    % HRTF
    if (get(handles.pca_mode,'Value') == 1) && (get(handles.add_mean,'Value') == 1)
    % subjects mean, all subjects
    hrtf_mag_left =  squeeze(mean(MEAN_SUB(:,1,1,:),1))' + dtf_reconstruct_left;
    hrtf_mag_right =  squeeze(mean(MEAN_SUB(:,1,2,:),1))' + dtf_reconstruct_right;
    end
    
    
    if (get(handles.pca_mode,'Value') == 1) && (get(handles.add_mean,'Value') == 2)
    % subjects mean, own mean
    hrtf_mag_left =  squeeze(MEAN_SUB(sol_sub,1,1,:))' + dtf_reconstruct_left;
    hrtf_mag_right =  squeeze(MEAN_SUB(sol_sub,1,2,:))' + dtf_reconstruct_right;
    end
    
    if (get(handles.pca_mode,'Value') == 2)
    % global mean    
    hrtf_mag_left =  squeeze(MEAN_SUB(1,1,1,:))' + dtf_reconstruct_left;
    hrtf_mag_right =  squeeze(MEAN_SUB(1,1,2,:))' + dtf_reconstruct_right;
    end
    ...
    reconstruct_left = reconstruct(1:half_points);
    reconstruct_right = reconstruct(half_points+1:end);
...
N = 1024;
freq = (0 : (N/2)-1) * fs / N;
hrtf_measured_left = 20*log10(2*abs(fft(squeeze(DB(sol_sub,pca_position,1,:)),N)));
...
N = 1024;
freq = (0 : (N/2)-1) * fs / N;
hrtf_measured_right = 20*log10(2*abs(fft(squeeze(DB(sol_sub,pca_position,2,:)),N)));
...
measured_hrtf_left = 20*log10(2*abs(fft(squeeze(DB(sol_sub,pca_position,1,:)),half_points*2)));

figure(3)
clf;
plot(freq,hrtf_mag_left,'r')
hold on
semilogx(freq,measured_hrtf_left(1:half_points),'b')

title('HRTF Left Reconstruction')
legend('HRTF left Reconstruction','HRTF left measured') 
grid on

rmse_left=rmse(measured_hrtf_left(1:half_points),hrtf_mag_left);
title(sprintf('HRTF Left Reconstruction with %2.2f dB RMS error',rmse_left))
legend('Measured','Reconstruction')
end
...
freq = (0 : (size(hrtf_mag_right,2))-1) * fs / (2*size(hrtf_mag_right,2));     
measured_hrtf_right = 20*log10(2*abs(fft(squeeze(DB(sol_sub,pca_position,2,:)),half_points*2)));
    
figure(6)
clf;
plot(freq,hrtf_mag_right,'r')
hold on
semilogx(freq,measured_hrtf_right(1:half_points),'b')
grid on

rmse_right=rmse(measured_hrtf_right(1:half_points),hrtf_mag_right);

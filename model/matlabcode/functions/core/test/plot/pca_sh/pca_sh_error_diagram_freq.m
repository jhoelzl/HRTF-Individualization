function pca_sh_error_diagram_freq()
...
for db=1:length(dbs)
    ...
    % Load Error File
    error_data = sprintf('../matlabdata/test_pca_sh/variance_error_pca_sh_%s_new_freq.mat',dbs{db});
    ...
    error_files = [3]; % only freq error files
                ...
                for sm = 1:length(conf.smoothing)
                    for em = 1:length(conf.ear_modes)
                        for ear = 1:length(conf.ears)
                            ...
                            saveas(11,sprintf('../thesis/images/test_pca_sh/boxplot_freq/%s_%s_ipm%i_ips%i_em%i_sm%i_pc%i_sh%i',dbs{db},conf.error_type{error_files(e_name)},conf.input_modes(im),conf.input_structures(is),conf.ear_modes(em),conf.smoothing(sm),conf.pc_numbers(pc),conf.sh_orders(sh)),'epsc');

function pca_sh_error_diagram_pcws
                    ...
                    for sm = 1:length(conf.smoothing)  
                        for em = 1:length(conf.ear_modes)   
                            for ear = 1:length(conf.ears)
                                ...
                                saveas(11,sprintf('../thesis/images/test_pca_sh/shape_error/%s_%s_ipm%i_ips%i_em%i_ear%s_sm%i_pc%i_sh%i',dbs{db},strrep(conf.error_type{error_files(e_name)},'.','_'),conf.input_modes(im),conf.input_structures(is),conf.ear_modes(em),name_ear,conf.smoothing(sm),conf.pc_numbers(pc),conf.sh_orders(sh)),'epsc');

function pca_compression_dbs_lines()
...
for db=1:size(X,1)
    ...
    plot(xax,squeeze(X(db,inp,xax)),'Color',col{db},'LineStyle',mark{inp},'Linewidth',2);
    hold on
...
%saveas(3,sprintf('../thesis/images/compression/pca_structur/pca_compression_dbs3_inp%i_em%i',input_struct,ear_mode),'epsc');

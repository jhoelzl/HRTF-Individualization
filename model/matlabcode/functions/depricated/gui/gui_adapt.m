function varargout = gui_adapt(varargin)

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @gui_adapt_OpeningFcn, ...
                   'gui_OutputFcn',  @gui_adapt_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function gui_adapt_OpeningFcn(hObject, eventdata, handles, varargin)
global subject_id
global ari_id
global eq_id

% Choose default command line output for gui_adapt
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes gui_adapt wait for user response (see UIRESUME)
...
adapt_mode_Callback(hObject, eventdata, handles,0)
select_db_Callback(hObject, eventdata, handles)

function varargout = gui_adapt_OutputFcn(~, ~, handles) 
...
if (~isempty(strfind(current_db,'ari'))) && (ari_id ~= 0)
...
global current_db
global angles
clearvars -global pca_input pca_mean pcs pc_weights pc_weights_rc ic_weights ics nnmf_weights nnmf_basis sh_weights sh_weights_rc sha shb e hrir_reconstr_left hrir_reconstr_right
global pc_weights
global pca_mean
global freq_mode
global pcs
...
if (~isempty(strfind(current_db,'ari'))) && (get(handles.include_own,'Value') == 0)
    sub_id_excl = ari_id;
    excluded_hrtf = 1;  
    subjects = subjects-1;
    subject_ids(ari_id) = [];
...
db_precalculated = 0;
freq_mode = 4;
algo_modus = get(handles.algo,'Value');

if (excluded_hrtf ==1)
    
    if (algo_modus == 1)
    % PCA    
    db_file = sprintf('../matlabdata/db_pcs/pc_%s_sub%i_freqmode%i_smooth%i_%i_ex%i.mat',current_db,subjects,freq_mode,smooth_val,get(handles.algo,'Value'),ari_id);
    else
    % ICA  and NNMF  
    db_file = sprintf('../matlabdata/db_pcs/pc_%s_sub%i_freqmode%i_smooth%i_algo%i_pc%i_ex%i.mat',current_db,subjects,freq_mode,smooth_val,get(handles.algo,'Value'),get(handles.pc_numbers,'Value'),ari_id);
    end
    
else
    if (algo_modus == 1)
    % PCA
    db_file = sprintf('../matlabdata/db_pcs/pc_%s_sub%i_freqmode%i_smooth%i_algo%i.mat',current_db,subjects,freq_mode,smooth_val,get(handles.algo,'Value'));  
    else
    % ICA and NNMF
    db_file = sprintf('../matlabdata/db_pcs/pc_%s_sub%i_freqmode%i_smooth%i_algo%i_pc%i.mat',current_db,subjects,freq_mode,smooth_val,get(handles.algo,'Value'),get(handles.pc_numbers,'Value'));   
    end
end
...
db_precalculated = 0; %change only to 0 for testing
...
if (db_precalculated == 0)
    ...
    pca_structur=2; 
    pca_mean_mode=2;
    [pca_input,pca_mean] = algo_input(hrirs(subject_ids,1:size(hrirs,2),ears,:),pca_mode,pca_structur,pca_mean_mode,ear_mode,smooth_val,fs);    
    
    % PCA
    setProcess(status_process,'PCA ...')
    [pcs,pc_weights,latent] = princomp(pca_input,'econ'); % both ears
    ...
    [pca_input,pca_mean] = algo_input(hrirs(1:subjects,1:size(hrirs,2),ears,:),pca_mode,pca_structur,pca_mean_mode,ear_mode,smooth_val,fs);
    
    % ICA
    setProcess(status_process,'ICA ...')
    [ics,ic_weights,W] = fastica(pca_input, 'numOfIC', get(handles.pc_numbers,'Value'),'lastEig', get(handles.pc_numbers,'Value'),'verbose','off','interactivePCA','off');
    
    % Save Precalculated ICA Data
    save(db_file,'ics','ic_weights','pca_mean');
    end   
    
    if (algo_modus == 3) % NNMF
       
    % NNMF Input Matrix
    setProcess(status_process,'NNMF Input Matrix ...')
    ...
    % NNMF
    setProcess(status_process,'NNMF ...')
    [nnmf_weights,nnmf_basis] = nnmf(pca_input,get(handles.pc_numbers,'Value'));
      
    % Save Precalculated ICA Data
    save(db_file,'nnmf_basis','nnmf_weights','pca_mean');
    end   
...
sound_Callback(hObject, eventdata, handles)
...
if (calc_status ==1) % only when PCA calculation was done before
    ...
    % Spherical Harmonics from PCA or ICA Weights
    setProcess(status_process,'Update GUI ...')
    ...
    if (algo_modus == 1) && ((adapt_mode == 2) || (adapt_mode == 3))
%         size(pc_weights)
%         size(angles)
%         pc_order
%         sh_order
...
% Create Button Group
button_group = uibuttongroup('SelectionChangeFcn', 'gui_adapt(''choose_pc'', gcbo, guidata(gcbo))','BackgroundColor',[0.6 0.6 0.6],'BorderType','none','Units','characters','Position',[33 0 21 36]);
...
pc_numbers = get(findobj('tag','pc_numbers'),'Value');
...
for pc = 1:pc_numbers   
    ...
    if (algo_modus ==2)
        pcno = sprintf('IC%i',pc);
        name_radio = sprintf('pc%i_radio',pc);
    end
    
    if (algo_modus ==3)
        pcno = sprintf('NNMF%i',pc);
        name_radio = sprintf('pc%i_radio',pc);
    end
    ...
    if (sh_modus == 1)
        switch(algo_modus)
            case 1 % Show PC Weights as Sliders
            posy1 = CreateSliders_PC2SH_PCW(pc,posy1,slid_range,color_ol);
            
            case 2 % Show IC Weights as Sliders
            posy1 = CreateSliders_PC2SH_ICW(pc,posy1,slid_range,color_ol);    
        
            case 3 % Show NNMF Weights as Sliders
            posy1 = CreateSliders_PC2SH_NNMF(pc,posy1,slid_range,color_ol);    
...
function [posy1] = CreateSliders_PC2SH_NNMF(pc,posy1,slid_range,color_ol)
global nnmf_weights

% Mean
nnmf_weights_mn = mean(nnmf_weights(:,pc));

% Std
nnmf_weights_std = std(nnmf_weights(:,pc));

name_sl = sprintf('slider%i',pc); 
posy1 = posy1-22;

% Min/Max values
min = (nnmf_weights_mn-slid_range*nnmf_weights_std);
max = (nnmf_weights_mn+slid_range*nnmf_weights_std);
val = nnmf_weights_mn;

% Create new Slider   
uicontrol('Tag',name_sl,'Backgroundcolor',[1 0 0],'Callback','gui_adapt(''slider_change_pc_nnmf'', gcbo, guidata(gcbo))','Style','slider','Units','pixels','Position', [390 posy1 200 50],'Min',min,'Max',max,'Value', val);



function [posy1] = CreateSliders_PC2SH_ICW(pc,posy1,slid_range,color_ol)
global ic_weights

% Mean
ic_weights_mn = mean(ic_weights(:,pc));

% Std
ic_weights_std = std(ic_weights(:,pc));

name_sl = sprintf('slider%i',pc); 
posy1 = posy1-22;

% Min/Max values
min = (ic_weights_mn-slid_range*ic_weights_std);
max = (ic_weights_mn+slid_range*ic_weights_std);
val = ic_weights_mn;

% Create new Slider   
uicontrol('Tag',name_sl,'Backgroundcolor',[1 0 0],'Callback','gui_adapt(''slider_change_pc_icw'', gcbo, guidata(gcbo))','Style','slider','Units','pixels','Position', [390 posy1 200 50],'Min',min,'Max',max,'Value', val);
...
% Create new Slider   
uicontrol('Tag',name_sl,'Backgroundcolor',[1 0 0],'Callback','gui_adapt(''slider_change_pc_pcw'', gcbo, guidata(gcbo))','Style','slider','Units','pixels','Position', [390 posy1 200 50],'Min',min,'Max',max,'Value', val);
...
pca_input = squeeze(sh_weights(:,:,pc));

%size(sh_weights)
% Substract Mean
mean_sub = mean(pca_input,2);
mean_sub = repmat(mean_sub,1,size(pca_input,2));
pca_input = pca_input - mean_sub;

% PCA
[cur_sh_pcs,cur_sh_pc_weights,cur_sh_latent] = princomp(pca_input,'econ');
...
for sh = 1:sh_pc_number
  ...
  % Create new Slider   
    uicontrol('Tag',name_sl,'Backgroundcolor',[1 0 0],'Callback','gui_adapt(''slider_change_pc2sh_pca'', gcbo, guidata(gcbo))','Style','slider','Units','pixels','Position', [390 posy1 200 50],'Min',min,'Max',max,'Value', val);
...
global ov_dir
    ...
    for sh = 1:size(sh_weights,2)
        ...
        % Create new Slider   
        uicontrol('Tag',name_sl,'Backgroundcolor',[1 0 0],'Callback','gui_adapt(''slider_change_pc2sh'', gcbo, guidata(gcbo))','Style','slider','Units','pixels','Position', [390 posy1 200 50],'Min',min,'Max',max,'Value', val);
...
% Create Button Group
button_group = uibuttongroup('SelectionChangeFcn', 'gui_adapt(''choose_sh'', gcbo, guidata(gcbo))','BackgroundColor',[0.6 0.6 0.6],'BorderType','none','Units','characters','Position',[33 0 21 36]);
...
for sh = 1:size(sh_weights,2)    
    ...
    uicontrol('Tag',name_radio,'Parent',button_group,'FontSize',12,'FontWeight','normal','String',shno,'Style','radiobutton','Units','pixels','Position', [10 posy 110 20]);
        ...
        % Create new Slider
        uicontrol('Tag',name_sl,'Callback','gui_adapt(''slider_change_sh2pc'', gcbo, guidata(gcbo))','Style','slider','Units','pixels','Position', [390 posy1 200 50],'Min',min,'Max',max,'Value', val);
...
function slider_change_pc_nnmf(hObject,~,~)
global nnmf_weights_rc
...
nnmf_weights_rc = slider_val(end:-1:1)';


reconstruction('adapt',0,1,pc_changed)

function slider_change_pc_icw(hObject,~,~)
global ic_weights_rc
...
slider_val  = get(h,'Value');
slider_val = cell2mat(slider_val);
ic_weights_rc = slider_val(end:-1:1)';
...
if (algo_modus == 2) % ICA
    reconstruct = ic_weights_rc * ics;
    reconstruct = reshape(reconstruct,[],size(hrirs,2));
    reconstruct = reconstruct(:,current_positions);
    
    % Left / Right
    len = size(reconstruct,1);
    reconstruct_left = squeeze(reconstruct(1:len/2,:))';
    reconstruct_right = squeeze(reconstruct(len/2+1:end,:))';
end

if (algo_modus == 3) % NNMF    
    
    reconstruct = nnmf_weights_rc * nnmf_basis;
    reconstruct = reshape(reconstruct,[],size(hrirs,2));
    reconstruct = reconstruct(:,current_positions);
...
if (ari_id == 0)  || (isempty(ari_id) == 1) || (~isempty(strfind(current_db,'ari'))) || (excluded_hrtf == 0)
    itd_id = 1;
else
    itd_id = ari_id;
end
...
if (strcmp(get(findobj('tag','uitool_plot_sh'),'State'),'on') == 1)
    plot_sh(sh_order,sh_weights_rc,angles,current_positions,'multi',current_db)
end

% Plot 3D SH weights (single window)
if (strcmp(get(findobj('tag','uitool_plot_sh_single'),'State'),'on') == 1)
    plot_sh(sh_order,sh_weights_rc,angles,current_positions,'single',current_db)
end
...
%size(hrir_reconstr_left)

play_sound(hrir_reconstr_left',hrir_reconstr_right',current_positions,fs,stim,rir,eq_id,str2num(get(findobj('tag','silence_value'),'String')),get(findobj('tag','play_simul'),'Value'))
...
play_sound(squeeze(hrirs(ari_id,current_positions,1,:)),squeeze(hrirs(ari_id,current_positions,2,:)),current_positions,fs,stim,rir,eq_id,str2num(get(findobj('tag','silence_value'),'String')),get(findobj('tag','play_simul'),'Value'))
...
if (~isempty(strfind(current_db,'kemar')))
    
    % play kemar HRIRs
    play_sound(squeeze(hrirs(1,current_positions,1,:)),squeeze(hrirs(1,current_positions,2,:)),current_positions,fs,stim,rir,eq_id,str2num(get(findobj('tag','silence_value'),'String')),get(findobj('tag','play_simul'),'Value'))
...
c_pos=findobj('tag','position');
[~,ind_daten] = trajectory_list(current_db,angles);
model.current_positions = ind_daten{get(c_pos,'Value')};
...
if (get(handles.slider_modus,'Value') == 1)
    ...
    if (get(handles.adapt_mode,'Value') == 1)
        switch(get(handles.algo,'Value'))
            ...
            case 2 % ICA
            ResetSliderPC_ICW(hObject, eventdata, handles)  
            case 3 % NNMF
            ResetSliderPC_NNMF(hObject, eventdata, handles)  
        end
    end
...
if (get(handles.slider_modus,'Value') == 1)
    ...
    if (get(handles.adapt_mode,'Value') == 1)
        switch(get(handles.algo,'Value'))
            ...
            case 2 % ICA
            ResetSliderPC_active_ICW(hObject, eventdata, handles) 
            case 3 % NNMF
            ResetSliderPC_active_NNMF(hObject, eventdata, handles)  
        end
    end
...
function adapt_mode_Callback(hObject, eventdata, handles,refresh)
...
switch(get(handles.algo,'Value'))
    case 1
    % PCA
    set(handles.adapt_mode,'String','INP2: Adapt PC Weights|INP2: Adapt SH Weights|INP2: Adapt SH Weights (PCA)')
    
    case 2
    % ICA
    set(handles.adapt_mode,'Value',1);
    set(handles.adapt_mode,'String','INP1: Adapt IC Weights')
    adapt_mode_Callback(hObject, eventdata, handles,0)
   
    case 3
    % NNMF
    set(handles.adapt_mode,'Value',1);
    set(handles.adapt_mode,'String','INP1: Adapt NNMF Weights')
    adapt_mode_Callback(hObject, eventdata, handles,0)   
end

function output_wave = mcreatehuggins2(transitioncf, transitionbw, lowfrequency, highfrequency, spectrumlevelleft, spectrumlevelright, itd, ipd, duration, gatelength, samplefreq, infoflag)
% function output_wave = ...
% mcreatehuggins2(transitioncf, transitionbw, 
%                 owfrequency, highfrequency, ...
...
% carried on a noise from 0 Hz to 2000 Hz, 1000-Hz bandwidth,
...
% abort if the lowerfreq < 0
if lowfrequency < 0 
   fprintf('%s: error! low frequency < 0\n\n', mfilename); 
   output_wave = [];
   return;
end;
   
% abort if the lowerfreq >= highfreq
if lowfrequency >= highfrequency
   fprintf('%s: error! low frequency >= high frequency\n\n', mfilename);
   output_wave = [];
   return;
end;
...
centerfreq = highfrequency - bandwidth/2;
